...
router.patch('/:id', checkPermission('ticket.close.all'), async (req: Request<{ id: string }, {}, UpdateTicketBody>, res: Response) => {
  try {
    ...
    // Handle appeal actions if provided
    if (updates.appealAction && ticket.type === 'appeal') {
      const appealAction = updates.appealAction;
      ...
      if (appealAction === 'pardon') {
        ...
        if (punishmentId && playerUuid && updates.customValues) {
          try {
            const Player = req.serverDbConnection!.model('Player');
            const player = await Player.findOne({ minecraftUuid: playerUuid, 'punishments.id': punishmentId });
            ...
            if (player) {
              ...
              if (punishment) {
                ...
                const newDuration = updates.customValues.duration;
                const isPermanent = updates.customValues.isPermanent;
